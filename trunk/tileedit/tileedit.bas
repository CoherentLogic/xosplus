...
ErrorSub:
    ...
    PRINT "? BACON <"; HEX$(ERR); "@"; HEX$(ERL); "> (OINK OINK)"
    SLEEP
    END
...
SUB DrawChrome ()
    ...
    ' DRAW HELP
    '
    X% = OutGText(500, 90, "KEYBOARD SHORTCUTS:")
    X% = OutGText(500, 90 + 16, "  S: Save this tile")
    X% = OutGText(500, 90 + 16 + 16, "  L: Load another tile")
    X% = OutGText(500, 90 + 16 + 16 + 16, "  R: Refresh display")
    X% = OutGText(500, 90 + 16 + 16 + 16 + 16, "  Q: Quit the editor")
...
SUB LoadTile ()
    ...
    PRINT ""
    PRINT ""
    LINE INPUT "ANSWER FILE TO EDIT (NO EXTENSION)? ", FilePath
    IF INSTR(FilePath, ".") > 0 THEN
        ...
        END
    END IF
    FilePath = FilePath + ".PIG"
    OPEN FilePath FOR BINARY AS #1
    GET #1, , Tiles
    CLOSE #1
    UpdateTrans = 1
    PRINT ""
    PRINT "TILE NAME:         ", Tiles.Tilename
    PRINT "COPYRIGHT:         ", Tiles.Copyright
    PRINT "TRANSPARENT COLOR: ", LTRIM$(STR$(Tiles.TransColor))
    PRINT ""
    PRINT ""
...
SUB SaveTile ()
    ...
    CLS
    LOCATE 10, 10
    LINE INPUT "ANSWER FILE TO SAVE (NO EXTENSION)? ", FilePath
    LOCATE 11, 10
